...
int main(void)
{
  ...
  /* -----------
  Most Useful Functions in the program
  LL_GPIO_WriteOutputPort(GPIO Port, Level value for each pin of the port) --> Writes to a port a specific byte code
  LL_GPIO_ReadOutputPort(GPIO Port) --> Returns the data register value of that port
  HAL_GPIO_ReadPin(GPIOx, GPIO_Pin) --> Reads the specified input port pin.
