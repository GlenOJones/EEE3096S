...
/**
 * Code Template for task 4 & 6. Very important to check comments
  ******************************************************************************
  * @file           : main.c
...
#include <stdint.h>
//------------------------------------
//Very IMPORTANT, Change this library to match the board you are using, either F0 or F4
#include "stm32f0xx.h"
//------------------------------------
...
// Struct to store different resolutions

typedef struct { uint16_t w, h; } res_t;

//TASK 4 RESOLUTIONS OF INTEREST
...
////TASK 6 RESOLUTIONS OF INTEREST
//static const res_t kResList[] = {
...
typedef struct {
  uint16_t w, h;
  uint32_t ms;			  // milliseconds taken, wall-clock time
  uint64_t checksum;	  // checksum value
  uint32_t flash_bytes;   // code+rodata+data-init in flash
  uint32_t ram_bytes;     // data+bss in SRAM at runtime
  uint32_t bin_bytes;     // approx .bin size (use flash_bytes)
} result_t;
...
//For task 8, we use the test case dimension of {640, 480}
  for (size_t r = 7; r < 8; ++r) {  // use r < kNumRes (task 4) or 5 (task 6)
